...
static bool compare_time (const struct list_elem *elementA, const struct list_elem *elementB, void *aux UNUSED)
{
  ...
  if ( thread2->wakeup > thread1->wakeup ){
...
int64_t start = timer_ticks ();
  ...
  current->wakeup = start + ticks;
    
  intr_disable ();
    
  list_insert_ordered (&asleep, &current->elem, compare_time, NULL);
...
/////////////////////////////////////////altered code
  ...
  while (!list_empty (&asleep)){

    temp = list_entry (list_front (&asleep), struct thread, elem);
